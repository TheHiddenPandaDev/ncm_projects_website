...
//                    'label'       => 'Activar Pago por Referencia',
    ...
    function process_payment( $order_id ) {
		...
		$userId = $order->get_meta('_customer_user', true);
    ...
    function generate_redsys_form( $order_id ) {
        ...
        if($allowReference){
            ...
            $idCustomer = $order->get_meta('_customer_user', true);
		...
		if($allowReference){
			$html .='<button class="button alt wp-element-button" type="submit" id="submit_redsys_payment_form">'.__('Pagar con Tarjeta', 'redsys').'</button>'. ' ' .
...
//                        $order->reduce_order_stock();
                        ...
                        if($this->get_option('withref') == 1 && is_user_logged_in()){
                            ...
                            if($reference!=null){
                                $idCustomer = $idCart->get_meta('_customer_user', true);
