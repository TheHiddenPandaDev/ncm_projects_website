...
//         $this->withdcc            = $this->get_option( 'withdcc' );
    ...
    function process_order() {
		...
		if ($this->with3ds) {
			...
			if ($version[0] == "1") {
				...
				$notificationURL = WC_Redsys_Insite::createEndpointParams($this->secure_back_v2_url, $request, $orderId, $ThreeDSInfo, $idLog);
		...
		if ($result->getResult () == RESTConstants::$RESP_LITERAL_AUT) {
			...
			if ($ThreeDSInfo == "1.0.2") {
				...
				$termURL = WC_Redsys_Insite::createEndpointParams($this->secure_back_url, $result->getOperation (), $orderId, null, $idLog);
				...
				$challengeUrl = $this->secure_redir_url;
			} else {

				WC()->session->set('REDSYS_creq', $result->getCreqParameter ());
				WC()->session->set('REDSYS_urlacs', $result->getAcsURLParameter ());

				$challengeUrl = $this->secure_redir_v2_url;
			}
			...
			set_transient('REDSYS_payment_result_' . $orderId, json_encode($paymentResult), 600);
			set_transient('REDSYS_payment_result_' . $idCart, json_encode($paymentResult), 600);
			
			escribirLog("DEBUG", $idLog, "Fijado transient con paymentResult para idCart " . $idCart, null, __METHOD__);
			escribirLog("DEBUG", $idLog, "Valor de paymentResult: " . json_encode($paymentResult), null, __METHOD__);
		}

		header('Content-Type: application/json');
		die(json_encode(array("challenge_url"=>$challengeUrl)));
    }
    ...
    function back_from_tdsecure() {
		...
		set_transient('REDSYS_payment_result_' . $_GET['order'], json_encode($paymentResult), 600);
		set_transient('REDSYS_payment_result_' . $origIdCart, json_encode($paymentResult), 600);
			
		escribirLog("DEBUG", $idLog, "Fijado transient con paymentResult para idCart " . $_GET['order'], null, __METHOD__);
		escribirLog("DEBUG", $idLog, "Valor de paymentResult: " . json_encode($paymentResult), null, __METHOD__);

		$form = '<p style="font-family: Arial; font-size: 16; font-weight: bold; color: black; align: center;">
					Procesando operación...
				</p>
				<script>window.top.top.placeOrder();</script>';
    ...
    function process_payment( $order_id ) {
		...
		$redsysOrder = get_post_meta($order_id, 'redsysOrder', true);

		$paymentResult = get_transient('REDSYS_payment_result_' . $redsysOrder);

		if (empty($paymentResult)) {

			escribirLog("ERROR", $idLog, "No se han encontrado datos en el transient usando la información de Woocommerce. Probando 5 superiores e inferiores...", null, __METHOD__);
			...
			do {
				...
				escribirLog("DEBUG", $idLog, "Obteniendo transient para orderID " . $order_id, null, __METHOD__);
				$paymentResult = get_transient('REDSYS_payment_result_' . $order_id);

				if (!empty($paymentResult)) {

					escribirLog("DEBUG", $idLog, "Encontrado transient: " . $paymentResult, null, __METHOD__);

					$paymentResult = json_decode($paymentResult, true);

					$controlCode = $paymentResult['controlCode'];
					$orderIdLog = $order_id . $this->fuc;
					$guarda = generateIdLog($this->activar_log, $this->logString . $paymentResult['cardNumber'], $orderIdLog);

					escribirLog("DEBUG", $idLog, "cc Propio | cc Generado  -  $controlCode | $guarda", null, __METHOD__);

					if ($controlCode == $guarda)
						$encontrado = true;
					else
						$paymentResult = "";
			...
			} while ( ($repeticiones <= 11) and (!$encontrado) );

			if (!$encontrado)
				escribirLog("ERROR", $idLog, "No se ha encontrado un transient con un paymentResult válido tras $repeticiones intentos", null, __METHOD__);

		} else {

			escribirLog("DEBUG", $idLog, "Datos contenidos en paymentResult: " . $paymentResult, null, __METHOD__);
			$paymentResult = json_decode($paymentResult, true);	
		}
...
//				WC_Redsys_Refund::saveOrderId((new WC_Redsys)->get_next_order_id() - 1, $paymentResult['order'], $paymentResult['amount']);
				...
				if($reference!=null){
					$idCustomer = $order_id->get_meta('_customer_user', true);
				...
				$redirectUrl = $urlOK ? $urlOK : $order->get_checkout_order_received_url(true);
			} else {
				$order->add_order_note( __('[REDSYS] Respuesta del SIS: ', 'woocommerce') . $respuestaSIS[0]);
    ...
    function payment_fields(){
		...
		try{
			...
			if(!empty(array_diff($required_fields, array_keys($post_data ? $post_data : array())))) {
