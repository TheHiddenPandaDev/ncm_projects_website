...
class Products extends Abstract_Field {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				$html .= sprintf(
					'<label class="wpo-checkbox" %7$s>
						...
						<div>
							%5$s
							<span class="wpo-price-container">(%6$s)</span>
						</div>
					</label>',
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				foreach ( $variations as $variation_id ) :
					...
					$html .= sprintf(
						...
						esc_attr( $variation->get_price_html() ),
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				$html   .= sprintf(
					'<label class="wpo-image-button" %9$s %11$s>
						<input type="checkbox" id="%1$s" name="%2$s[%3$s][]" value="%3$s" %10$s data-price-type="flat_fee" data-price-amount="%6$s">
						<figure class="%8$s">
		...
		foreach ( $variations as $variation_id ) {
			...
			$html .= sprintf(
				'<label class="wpo-checkbox" %8$s>
					...
					<div>
						%6$s
						<span class="wpo-price-container">(%7$s)</span>
					</div>
				</label>',
		...
		foreach ( $variations as $variation_id ) {
			...
			$html .= sprintf(
				'<label class="wpo-image-button" %10$s %12$s>
					<input type="checkbox" id="%1$s" name="%2$s[%3$s][]" value="%4$s" %11$s data-price-type="flat_fee" data-price-amount="%7$s">
					<figure class="%9$s">
	...
	private function get_figcaption( $product ) {
		...
		$price = sprintf( '<span class="price wpo-price-container">%s</span>', $product->get_price_html() );
