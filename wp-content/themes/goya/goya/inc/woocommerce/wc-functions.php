...
---------------------------------------------------------- */
    ...
    function overwrite_shortcode() {
        ...
        function override_goya_progress_bar_amount_left($atts) {
            $threshold = get_theme_mod('progress_bar_goal', 0);

            $subtotal = recalculate_subtotal(WC()->cart->get_subtotal());
        ...
        remove_shortcode('missing_amount');
        add_shortcode ('missing_amount', 'override_goya_progress_bar_amount_left');
    }
	...
	function goya_progress_bar_content() {
        ...
        // $subtotal = recalculate_subtotal($subtotal);
    ...
    function recalculate_subtotal($subtotal) {

        $subtotal = floatval($subtotal);

        $product_conditions = [];
        $condition_groups = get_post_meta( '27607', '_wafs_shipping_method_conditions', true );
        foreach ( $condition_groups as $condition_group => $conditions ) :
            foreach ( $conditions as $condition_id => $condition ) :
                $product_conditions[] = $condition['value'];
            endforeach;
        endforeach;
        ...
        foreach ($user_cart as $cart_item) {
            ...
            if(in_array( $cart_item['product_id'], $product_conditions )){
                ...
                if (get_theme_mod('progress_bar_subtotal_taxes', true)) {
                    $product_price = floatval(wc_get_price_including_tax($product));
                } else {
                    $product_price = floatval(wc_get_price_excluding_tax($product));
                }
...
---------------------------------------------------------- */
	...
	function goya_sizing_guide_content() {
		$size_page = goya_sizing_guide();
		$mode = apply_filters('goya_size_guide_open_mode', 'lightbox');
		if (isset($size_page) && $mode == 'lightbox') { ?>
