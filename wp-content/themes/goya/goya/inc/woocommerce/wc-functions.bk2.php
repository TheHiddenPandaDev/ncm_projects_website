...
---------------------------------------------------------- */
	...
	function goya_progress_bar_content() {
		...
		if ( $subtotal < $threshold && !free_cart_match()) {
    ...
    function free_cart_match() {

        $product_carts = get_compare_value();
        $product_conditions = [];

        $condition_groups = get_post_meta( '26242', '_wafs_shipping_method_conditions', true );
        ...
        foreach ( $product_carts as $product ) :
            if( ! in_array( $product, $product_conditions )) return false;
        endforeach;
